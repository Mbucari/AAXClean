...
public class TrunBox : FullBox
{
    ...
    private bool sample_duration_present => (Flags & 0x100) == 0x100;
    ...
    public TrunBox(Stream file, BoxHeader header, IBox parent) : base(file, header, parent)
    {
        ...
        {
            int? sampleDuration = sample_duration_present ? file.ReadInt32BE() : null;
    ...
    protected override void Render(Stream file)
    {
        ...
        for (int i = 0; i < Samples.Length; i++)
        {
            if (sample_duration_present)
                file.WriteInt32BE(Samples[i].SampleDuration ?? 0);
    ...
    public class SampleInfo
    {
        ...
        public SampleInfo(int? sampleDuration, int? sampleSize, uint? sampleFlags, int? sampleCompositionTimeOffset)
        {
