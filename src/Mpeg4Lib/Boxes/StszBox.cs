...
public class StszBox : FullBox, IStszBox
{
	...
	public int SampleCount => sampleSizes_32?.Count ?? sampleSizes_16?.Count ?? 0;
	public int MaxSize => sampleSizes_32?.Max() ?? sampleSizes_16?.Max() ?? 0;
	public long TotalSize => sampleSizes_32?.Sum(s => (long)s) ?? sampleSizes_16?.Sum(s => (long)s) ?? 0;
	public int GetSizeAtIndex(int index) => sampleSizes_32?[index] ?? sampleSizes_16?[index] ?? 0;
	public long SumFirstNSizes(int firstN) => sampleSizes_32?.Take(firstN).Sum(s => (long)s) ?? sampleSizes_16?.Take(firstN).Sum(s => (long)s) ?? 0;
		...
		: base(file, header, parent)
	{
		...
		for (int i = 0; i < sampleCount; i++)
		{
			...
			if (value > ushort.MaxValue && sampleSizes_32 is null)
			{
				...
				sampleSizes_32 = new(sampleSizes_16.Select(s => (int)s));
				sampleSizes_16.Clear();
			}
