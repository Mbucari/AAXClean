...
namespace AAXClean.Util
{
    class ByteUtil
    {
        ...
        internal static byte[] CloneBytes(byte[] src)
        {
            return CloneBytes(src, 0, src.Length);
        }
        internal static byte[] CloneBytes(byte[] src, int srcOffset, int count)
        {
            byte[] dst = new byte[count];
            Buffer.BlockCopy(src, srcOffset, dst, 0, count);
            return dst;
        }
        internal static bool BytesEqual(byte[] array1, byte[] array2, bool reverseDirection = false)
        {
            return BytesEqual(array1, 0, array2, 0, array1.Length, reverseDirection);
        }
        internal static bool BytesEqual(byte[] array1, int startIndex1, byte[] array2, int startIndex2, int count, bool reverseDirection = false)
        {
